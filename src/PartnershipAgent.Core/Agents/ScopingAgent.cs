...
/// <summary>
/// The ScopingAgent determines whether a user's request is within the scope of the
/// partnership agent system. It filters out off-topic requests, jokes, and other
/// non-partnership-related queries.
/// </summary>
public class ScopingAgent : BaseChatHistoryAgent, IScopingAgent
{
    ...
    /// <summary>
    /// Constructor for the ScopingAgent.
    ...
    private void InitializeAgent()
    {
        ...
        var executionSettings = new OpenAIPromptExecutionSettings
        {
            ...
            ResponseFormat = typeof(ScopingAgentResponse)
        };
    ...
    public async Task<ScopingAgentResponse> EvaluateScopeAsync(string prompt)
    {
        ...
        try
        {
            ...
            chatHistory.AddUserMessage(agentMessage);

            var responseList = new List<ChatMessageContent>();
            ...
            var lastMessage = assistantMessage?.Content ?? string.Empty;
            ...
            try
            {
                var response = JsonSerializer.Deserialize<ScopingAgentResponse>(
                    lastMessage,
                ...
                Logger.LogInformation("Scope evaluation complete: IsInScope={IsInScope}, Category={Category}",
                    response.IsInScope, response.Category);

                return response;
            }
            catch (JsonException jsonEx)
            {
                Logger.LogWarning(jsonEx, "Failed to deserialize scoping response. Content: {Content}", lastMessage);
                return CreateDefaultInScopeResponse();
            }
        }
    ...
    /// <summary>
    /// Determines the scope of a request (kernel function wrapper).
    /// This method is exposed as a KernelFunction for the agent to use.
    /// </summary>
    /// <param name="prompt">The user's request to evaluate</param>
    /// <returns>Scoping analysis result</returns>
    [KernelFunction, Description("Determines if a user request is in scope for the partnership agent system.")]
    public async Task<ScopingAgentResponse> DetermineScope(
        [Description("The user's request to evaluate")] string prompt)
    {
        return await EvaluateScopeAsync(prompt);
    }
